  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-12">
        ...
        <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-3">Protect Your Properties, Retain Your Tenants</h1>
        ...
        {BENEFITS.map((b, i) => (
          <div key={i} className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="font-bold text-gray-900 mb-1">{b.title}</h2>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-blue-50 rounded-2xl border border-blue-100 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="font-bold text-gray-900 text-lg mb-2">Embeddable "Safety Verified" Badge</h2>
              <p className="text-gray-600 text-sm mb-4">Embed a live safety badge on your property listing website to show tenants your commitment to safety. The badge links directly to ElectroSafe.homes verification page.</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="font-bold text-gray-900 text-lg">Tenant Safety Notice Generator</h2>
              <p className="text-gray-500 text-sm">Fill in details to generate a printable, professional tenant safety communication letter.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Property Name</label>
              <input type="text" placeholder="e.g. Sunrise Apartment 3B" value={propName} onChange={e => setPropName(e.target.value)}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Property Address</label>
              <input type="text" placeholder="e.g. 123 Main Street, City" value={propAddress} onChange={e => setAddress(e.target.value)}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Tenant Name</label>
              <input type="text" placeholder="e.g. Mr./Ms. Kumar" value={tenantName} onChange={e => setTenantName(e.target.value)}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Inspection Date</label>
              <input type="date" value={inspDate} onChange={e => setInspDate(e.target.value)}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none text-sm" />
            </div>
            <div className="sm:col-span-2">
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block mb-1">Inspector / Landlord Name</label>
        ...
        {/* Generated Letter */}
        {showLetter && (
          <div className="bg-white rounded-2xl border-2 border-blue-200 shadow-lg p-8 print-content">
            <div className="border-b-2 border-gray-800 pb-4 mb-6">
              <div className="flex items-center gap-2 mb-2">
                <Zap className="w-5 h-5 text-blue-600" />
                <span className="font-black text-gray-900">ElectroSafe.homes — Verified Safety Notice</span>
              </div>
              <p className="text-xs text-gray-500">electrosafe.homes | Electrical Safety Standards: IEC 60364 / NEC 2023 / BS 7671</p>
            </div>

            <p className="text-gray-600 text-sm mb-4">{today}</p>
            <p className="text-gray-800 font-medium">Dear {tenantName || '[Tenant Name]'},</p>
            <br />
            ...
            <div className="border-t border-gray-200 pt-4">
              <p className="text-sm font-bold text-gray-800">{inspector || '[Inspector/Landlord Name]'}</p>
              <p className="text-xs text-gray-500">{propName || '[Property Name]'} — Safety verified via ElectroSafe.homes</p>
            </div>
